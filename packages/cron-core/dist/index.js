...
export const SECOND_DEF = { key: "second", label: "초 (Second)", min: 0, max: 59 };
export const YEAR_DEF = { key: "year", label: "연도 (Year)", min: 2024, max: 2099 };
export const ALL_FIELD_DEFS = [
    SECOND_DEF,
    ...COMMON_FIELDS,
    YEAR_DEF,
].reduce((acc, def) => {
...
export function defaultField(def) {
    return {
        mode: "every",
        specific: [],
        rangeStart: def.min,
...
export function makeFields(platform, overrides) {
    const result = {};
    for (const def of FIELD_DEFS[platform]) {
        result[def.key] = { ...defaultField(def), ...overrides[def.key] };
    }
    for (const key of ["second", "minute", "hour", "day", "month", "weekday", "year"]) {
        if (!result[key]) {
...
export function fieldToExpression(field, def) {
    switch (field.mode) {
        case "every":
            return "*";
        case "specific":
...
export function buildExpression(fields, platform) {
    return FIELD_DEFS[platform].map((def) => fieldToExpression(fields[def.key], def)).join(" ");
}
export function describeExpression(fields, platform) {
    const parts = [];
    for (const def of FIELD_DEFS[platform]) {
...
export function validateExpression(expr, platform) {
...
export function resolveJenkinsH(expr, hashSeed, defs) {
    const parts = expr.trim().split(/\s+/);
    if (parts.length !== defs.length) {
        return expr;
    }
...
export function getNextExecutions(expr, platform, count, hashSeed) {
    const defs = FIELD_DEFS[platform];
    ...
    const matches = (value, allowed) => allowed == null || allowed.includes(value);
    const results = [];
    const now = new Date();
